...
def single_res(request, id):
    ...
    ledger = TransactionTable(Transaction.objects.filter(resident=id).order_by('-submission_date'), exclude=('submission_date', 'full_name', 'last_update'))
...
def edit_meeting(request, id):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            form = HouseForm(instance=meeting)
    return render(request, 'admin/forms.html', locals())
